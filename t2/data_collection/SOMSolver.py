...
class Neuron:
    ...
    def update_goal_position(self, goal, radius):
        if ((self.position[0] - goal[0])**2 + (self.position[1] - goal[1])**2) <= radius**2:
        ...
        cx, cy = self.position
        # vector = goal - neuron
        # norm(vector*k) = radius
        # k = radius / norm(vector)
        v = [cx - px, cy - py]
        v = np.array(v)
        k = radius / np.linalg.norm(v)
        goal_pos = v*k
        self.goal_position = goal_pos + goal
        return self.goal_position
...
class SOMSolver:
    ...
    def select_winner(self, goal_idx):
        ...
        for idx, neuron in enumerate(self.neurons):
            if neuron.goal_idx is None:
                x, y = neuron.position
                distance = np.sqrt((act_goal_x - x) ** 2 + (act_goal_y - y) ** 2)
    ...
    def learn_epoch(self, learning_epoch):
        ...
        # choose random order of the goals
        ...
        for goal_idx in order:
            ...
            for i, dist in enumerate(distances):
                choice = winner_idx + dist
                choice = self.M + choice
                choice = choice % self.M
                neighbour = self.neurons[choice]
                neighbour.adapt_neuron(betas[i], alternate_goal)
